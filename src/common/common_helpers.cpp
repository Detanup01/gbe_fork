...
KillableWorker::KillableWorker(std::function<bool(void *)> thread_job,
...
KillableWorker::~KillableWorker() { kill(); }
...
void KillableWorker::thread_proc(void *data) {
  ...
  if (initial_delay.count() > 0) {
    ...
    if (kill_thread_cv.wait_for(lck, initial_delay, [this] {
          return this->kill_thread ||
                 (this->should_kill && this->should_kill());
        })) {
      return;
    }
  }
  ...
  while (1) {
    if (polling_time.count() > 0) {
      std::unique_lock lck(kill_thread_mutex);
      if (kill_thread_cv.wait_for(lck, polling_time, [this] {
            return this->kill_thread ||
                   (this->should_kill && this->should_kill());
          })) {
        return;
...
void KillableWorker::kill() {
  if (!thread_job || !thread_obj.joinable())
...
bool common_helpers::create_dir(std::string_view directory_path) {
  const auto parent(std::filesystem::u8path(directory_path).parent_path());
  return create_dir_impl(parent);
}

bool common_helpers::create_dir(std::wstring_view directory_path) {
  const auto parent(std::filesystem::path(directory_path).parent_path());
  return create_dir_impl(parent);
}
...
bool common_helpers::starts_with_i(std::string_view target,
...
bool common_helpers::starts_with_i(std::wstring_view target,
...
bool common_helpers::ends_with_i(std::string_view target,
                                 std::string_view query) {
  if (target.size() < query.size()) {
    return false;
  }
...
bool common_helpers::ends_with_i(std::wstring_view target,
                                 std::wstring_view query) {
  if (target.size() < query.size()) {
    return false;
  }
...
std::string
common_helpers::uint8_vector_to_hex_string(const std::vector<uint8_t> &v) {
  std::string result{};
...
bool common_helpers::str_cmp_insensitive(std::string_view str1,
...
bool common_helpers::str_cmp_insensitive(std::wstring_view str1,
...
std::string common_helpers::to_lower(std::string_view str) {
  ...
  std::transform(str.begin(), str.end(), _str.begin(),
...
std::wstring common_helpers::to_lower(std::wstring_view wstr) {
  ...
  std::transform(wstr.begin(), wstr.end(), _wstr.begin(),
...
to_absolute_impl(const std::filesystem::path &path,
...
std::string common_helpers::to_absolute(std::string_view path,
                                        std::string_view base) {
  if (path.empty())
    return {};
  auto path_abs =
      to_absolute_impl(std::filesystem::u8path(path),
                       base.empty() ? std::filesystem::current_path()
                                    : std::filesystem::u8path(base));
  auto u8str = path_abs.u8string();
  return std::string(u8str.begin(), u8str.end());
}

std::wstring common_helpers::to_absolute(std::wstring_view path,
                                         std::wstring_view base) {
  if (path.empty())
    return {};
  auto path_abs =
      to_absolute_impl(std::filesystem::path(path),
                       base.empty() ? std::filesystem::current_path()
...
bool common_helpers::file_size(const std::filesystem::path &filepath,
                               size_t &size) {
...
std::wstring common_helpers::to_wstr(std::string_view str) {
  ...
  if (str.empty() || !utf8::is_valid(str.begin(), str.end())) {
    return {};
  }
...
std::string common_helpers::str_replace_all(std::string_view source,
                                            std::string_view substr,
                                            std::string_view replace,
                                            bool case_insensitive) {
  if (source.empty() || substr.empty())
    return std::string(source);

  std::string out{};
  out.reserve(source.size() /
              4); // out could be bigger or smaller than source, start small
...
size_t common_helpers::str_find(std::string_view str_src,
                                std::string_view str_query, size_t start,
                                bool case_insensitive) {
  ...
  if (str_src.empty() != str_query.empty())
    return static_cast<size_t>(-1);
  if (str_src.empty() && str_query.empty())
    return start;
  if (str_src.size() < str_query.size())
    return static_cast<size_t>(-1);

  const auto cmp_fn =
      case_insensitive
          ? [](const char c1,
               const char c2) { return std::toupper(c1) == std::toupper(c2); }
...
std::vector<std::string> common_helpers::str_split(std::string_view str,
