...
#ifdef EMU_OVERLAY
        if (overlay) overlay->OpenOverlay(pchDialog);
#endif
    }
}
...
void Steam_Friends::ActivateGameOverlayToWebPage( const char *pchURL, EActivateGameOverlayToWebPageMode eMode )
{
    ...
    std::lock_guard<std::recursive_mutex> lock(global_mutex);
#ifdef EMU_OVERLAY
    if (overlay) overlay->OpenOverlayWebpage(pchURL);
#endif
}
...
#ifdef EMU_OVERLAY
    if (overlay) overlay->OpenOverlayInvite(steamIDLobby);
#endif
}
...
#ifdef EMU_OVERLAY
    if (overlay) overlay->OpenOverlay("Friends");
#endif
}
...
#ifdef EMU_OVERLAY
                if (overlay) overlay->FriendDisconnect(*f);
#endif
                friends.erase(f);
            }
        }
...
#ifdef EMU_OVERLAY
                if (overlay) overlay->FriendConnect(msg->friend_());
#endif
    ...
    if (msg->has_friend_messages()) {
...
#ifdef EMU_OVERLAY
                if (overlay && overlay->Ready() && !settings->hasOverlayAutoAcceptInviteFromFriend(msg->source_id()))
                {
                    //TODO: the user should accept the invite first but we auto accept it because there's no gui yet
                    // Then we will handle it !
                    if (overlay) overlay->SetLobbyInvite(*find_friend(static_cast<uint64>(msg->source_id())), msg->friend_messages().lobby_id());
                }
                else
#else
                if (!settings->hasOverlayAutoAcceptInviteFromFriend(msg->source_id()))
#endif
                {
        ...
        if (msg->friend_messages().type() == Friend_Messages::GAME_INVITE) {
            ...
            if (!settings->hasOverlayAutoAcceptInviteFromFriend(msg->source_id()))
            {
                ...
                // Then we will handle it !
#ifdef EMU_OVERLAY
                if (overlay) overlay->SetRichInvite(*find_friend(static_cast<uint64>(msg->source_id())), msg->friend_messages().connect_str().c_str());
#endif
            }
            else
            {
