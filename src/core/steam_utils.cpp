...
void Steam_Utils::SetOverlayNotificationPosition( ENotificationPosition eNotificationPosition )
{
    ...
    std::lock_guard<std::recursive_mutex> lock(global_mutex);
#ifdef EMU_OVERLAY
    if (overlay) overlay->SetNotificationPosition(eNotificationPosition);
#endif
}
...
bool Steam_Utils::IsOverlayEnabled()
{
    ...
    std::lock_guard<std::recursive_mutex> lock(global_mutex);
#ifdef EMU_OVERLAY
    if (overlay && overlay->Ready()) return true;
    else
#endif
    if (settings->auto_send_any_overlay_invites || settings->overlayAutoSendInvitesCount() > 0) return true;
...
bool Steam_Utils::BOverlayNeedsPresent()
{
    ...
    std::lock_guard<std::recursive_mutex> lock(global_mutex);
#ifdef EMU_OVERLAY
    if (overlay) return overlay->NeedPresent();
#endif
    return false;
}
...
void Steam_Utils::SetOverlayNotificationInset( int nHorizontalInset, int nVerticalInset )
{
    ...
    std::lock_guard<std::recursive_mutex> lock(global_mutex);
#ifdef EMU_OVERLAY
    if (overlay) overlay->SetNotificationInset(nHorizontalInset, nVerticalInset);
#endif
}
