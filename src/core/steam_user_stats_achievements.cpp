...
#ifdef EMU_OVERLAY
            if (overlay) overlay->AddAchievementNotification(internal_name, user_achievements[internal_name], false);
#endif
...
#ifdef EMU_OVERLAY
            if (overlay) overlay->AddAchievementNotification(actual_ach_name, user_achievements[actual_ach_name], true);
#endif
        }
    } catch (...) {}
