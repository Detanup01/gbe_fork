...
-- Project steamclient_hybrid - Stable steamclient64.dll WITH userdata creation
---------
project "steamclient_hybrid"
    kind "SharedLib"
    location "%{wks.location}/%{prj.name}"
    targetdir("build/" .. os_iden .. "/%{_ACTION}/%{cfg.buildcfg}/hybrid/%{cfg.platform}")
    ...
    -- defines - HYBRID: steamclient + api functions for userdata creation
    ---------
    filter {} -- reset the filter and remove all active keywords
    defines { -- added to all filters, later defines will be appended
        "STEAMCLIENT_DLL", "STEAMCLIENT_HYBRID", "EMU_EXPERIMENTAL_BUILD",
    }
